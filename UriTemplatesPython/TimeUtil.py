...
class TimeUtil:
        ...
        if len(time) == 4:
            ...
            if time.startswith('now'):
                ...
                if m!=None:
                    ...
                    id = max(1,idigit)
                    while id < TimeUtil.DATE_DIGITS:  # J2J for loop
                        n[id] = 1
                        id = id + 1

                    id = max(TimeUtil.DATE_DIGITS,idigit)
                    while id < TimeUtil.TIME_DIGITS:  # J2J for loop
                        n[id] = 0
                        id = id + 1
    ...
    def parseISO8601TimeRange(stringIn):
        ...
        if ss[0].startswith('P'):
            ...
            i = 0
            while i < TimeUtil.TIME_DIGITS:  # J2J for loop
